...
using create_func = std::function<file_handler_ptr(const istream_ptr&, const std::string&)>;

// Can the provider be created for this setup
using supports_func = std::function<bool(const istream_ptr&, const std::string&)>;

struct factory_entry {
    file_handler_tag tag;
    create_func creator;
    supports_func supports = [](const istream_ptr&, const std::string&) { return false; };
...
class file_handler_factory {
    ...
    // Create by id
    static file_handler_ptr create(size_t id, const istream_ptr& stream, const std::string& path);
    // Create by name
    static file_handler_ptr create(const std::string& name, const istream_ptr& stream, const std::string& path);
    ...
    static size_t supports(const istream_ptr& stream, const std::string& path);

    // Also get the handler's tag
    static size_t supports(const istream_ptr& stream, const std::string& path, file_handler_tag& tag);
